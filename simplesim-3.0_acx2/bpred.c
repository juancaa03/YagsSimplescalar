...
#include <math.h>
#include <assert.h>
#include <stdint.h>         //Para poder pasar un puntero a unsigned int y coger sus 6bits bajos(Yags)
...
bpred_create(enum bpred_class class,	/* type of predictor to create */
       ...
       unsigned int t_size,   //tamaño de la tabla de taken for Yags------------------------------------------Yags 
       unsigned int nt_size)  //tamaño de la tabla de not taken for Yags ------------------------------------------Yags
{
  struct bpred_t *pred;
...
case BPredYags:   // Yags -------------------------------------------------------------------------------------------------
	...
	fatal("level-2 size, `%d', must be non-zero and a power of two(4, 16, 64, 256, 1024)", 
      ...
      if (shift_width != 1 && shift_width != 2 && shift_width != 3 && shift_width != 4 && shift_width != 5)
  ...
  fatal("xor value, '%d', must be 0");
	...
	fatal("cannot allocate second level table");
      ...
      for (cnt = 0; cnt < l2size; cnt++)
	      pred_dir->config.two.l2table[cnt] = 2;
	    
      //Hasta aqui es como el 2level, ahora inicialización de taken PHT y not taken PHT

      pred_dir->config.two.t_size = t_size;

      pred_dir->config.two.nt_size = nt_size;

      pred_dir->config.two.taken_table = calloc(t_size, sizeof(unsigned char));
      if (!pred_dir->config.two.taken_table)
        fatal("cannot allocate taken table");
      
      for(int cnt = 0; cnt < t_size; cnt++)
        pred_dir->config.two.taken_table[cnt] = 254;

      pred_dir->config.two.nottaken_table = calloc(nt_size, sizeof(unsigned char));
        if (!pred_dir->config.two.nottaken_table)
        fatal("cannot allocate not taken table");

      for(int cnt = 0; cnt < nt_size; cnt++)
        pred_dir->config.two.nottaken_table[cnt] = 254;
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPredYags:               // Yags---------------------------------------------------------------
    fprintf(stream,
      "pred_dir: %s: YAGS: %d l1-sz, %d bits/ent, %d xor, %d l2-sz, %d T-NT-PHT-sz, direct-mapped\n",
      name, pred_dir->config.two.l1size, pred_dir->config.two.shift_width, pred_dir->config.two.xor, pred_dir->config.two.l2size, pred_dir->config.two.t_size);
    break;
...
/* predicts a branch direction CAMBIO PARA YAGS*/
struct bpred_lookup_t						/* struct of pointers to counter */
bpred_dir_lookup(struct bpred_dir_t *pred_dir,	/* branch dir predictor inst */
		 md_addr_t baddr)		/* branch address */
{
  struct bpred_lookup_t result = {NULL, NULL, NULL};  //Struct de punteros, añadidos los 2 de Yags
  ...
  switch (pred_dir->class) {
	...
	int l1index, l2index;
	...
	else
	  {
        ...
        result.p = (char *) &pred_dir->config.two.l2table[l2index];
      }
      break;
    case BPredYags:                     // Nuevo caso para el predictor YAGS
      {
        int l1index, l2index;
        ...
        // Índices directos para TakenPHT y NotTakenPHT
        int tpht_index = l2index & (pred_dir->config.two.t_size - 1);
        int ntpht_index = l2index & (pred_dir->config.two.nt_size - 1);

        // Punteros a TakenPHT y NotTakenPHT
        result.p_taken = (char *) &pred_dir->config.two.taken_table[tpht_index];
        result.p_nottaken = (char *) &pred_dir->config.two.nottaken_table[ntpht_index];
      }
      break;
    case BPred2bit:
      result.p = (char *) &pred_dir->config.bimod.table[BIMOD_HASH(pred_dir, baddr)];
      break;
    case BPredTaken:
    case BPredNotTaken:
      break;
					 ...
					 * used on mispredict recovery */
{
  ...
  //Yags
  struct bpred_lookup_t dir_lookup;
  int hit_taken = 0, hit_nottaken = 0;
  ...
  switch (pred->class) {
	...
	{
    struct bpred_lookup_t dir_lookup = bpred_dir_lookup (pred->dirpred.bimod, baddr);     //Dejamos tal cual el flujo anterior
	...
	{
      struct bpred_lookup_t dir_lookup = bpred_dir_lookup (pred->dirpred.bimod, baddr);     //Dejamos tal cual el flujo anterior
	  dir_update_ptr->pdir1 = dir_lookup.p;
	}
      break;
  ...
  case BPredYags:                               // Nuevo caso para el predictor YAGS
    if ((MD_OP_FLAGS(op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND)) {
      ...
      unsigned int l2index = pred->dirpred.twolev->config.two.shiftregs[l1index] |
                               ((baddr >> MD_BR_SHIFT) << pred->dirpred.twolev->config.two.shift_width);
      ...
      if (prev_t_tag == pc_tag) hit_taken = 1;
      if (prev_nt_tag == pc_tag) hit_nottaken = 1;
  ...
  //Yags
  int aux = *(dir_update_ptr->pdir1) & 0x3;
  if (pbtb == NULL && (pred->class == BPredYags && (hit_taken || hit_nottaken)))
  { 
    return ((aux >= 2)
      ? /* taken */ 1
      : /* not taken */ 0);
  }
  else if (pred->class == BPredYags && (hit_taken || hit_nottaken))
  {
    return ((aux >= 2)
      ? /* taken */ pbtb->target
      : /* not taken */ 0);
  ...
  if (pbtb == NULL)                       // Reutilizado para Yags(PHT principal)
    {
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  int index, i;
  unsigned char updated_counter = 0;    // Variable para saber si se actualizó el contador en la tabla auxiliar de Yags
...
/* Actualización para YAGS */
    if (pred->class == BPredYags) {
        ...
        if ((dir_update_ptr->pdir1 == pred->dirpred.twolev->config.two.taken_table || 
          ...
          tag_with_counter = *(dir_update_ptr->pdir1);
          counter = tag_with_counter & 0x3; // 2 bits bajos (contador)
        ...
        if (!correct && updated_counter == 0) {
          if (!pred_taken && taken) {
              // Fallo: PHT predijo NT pero fue T -> insertar en TakenPHT
              index = (baddr >> MD_BR_SHIFT) & (pred->dirpred.twolev->config.two.t_size - 1);
              pred->dirpred.twolev->config.two.taken_table[index] = ((pc_tag << 2) | 0x2); // Tag + Estado débilmente tomado
              updated_counter = 1;
          } else if (pred_taken && !taken) {
              // Fallo: PHT predijo T pero fue NT -> insertar en NotTakenPHT
              index = (baddr >> MD_BR_SHIFT) & (pred->dirpred.twolev->config.two.nt_size - 1);
              pred->dirpred.twolev->config.two.nottaken_table[index] = ((pc_tag << 2) | 0x1); // Tag + Estado débilmente no tomado
              updated_counter = 1;
          }
        }
    }                   // Yags -------------------------------------------------------------
  ...
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->class == BPred2Level || pred->class == BPredComb || pred->class == BPredYags))       // Añadimos Yags, para la actualización de l1
    {
  ...
  if (dir_update_ptr->pdir1 && updated_counter == 0)  // Añadido updated_counter para Yags
    {
      if (taken)
	{
  ...
  if (dir_update_ptr->pdir2 && updated_counter == 1)
    {
      if (taken)
	{
