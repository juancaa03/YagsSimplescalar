...
struct bpred_dir_t {
  ...
  union {
    ...
    struct {
      ...
      int t_size;     //tamaño de la tabla de taken ------------------------------------------Yags
      int nt_size;    //tamaño de la tabla de not taken ------------------------------------------Yags
      unsigned char *taken_table; //tabla de taken --------------------------------------------------Yags
      unsigned char *nottaken_table;  //tabla de not taken ------------------------------------------Yags
    } two;
  } config;
};
...
/* Added Yags branch predictor dir_lookup information (Yags pointers)*/
struct bpred_lookup_t {
  char *p;           // Puntero principal
  char *p_taken;     // Puntero a PHT Taken
  char *p_nottaken;  // Puntero a PHT NotTaken
};
